...
def collectOverviewData(x, y, r, m, f, t, results):
    # Overview data:
    # Time
    # C, Cm, Csp
...
def plotOverviewData(df):
    ...
    # Fault
    df['Sensor C fault'][df['Sensor C fault'] == 0] = np.nan
    ax[0].plot(-df['Sensor C fault']*fault_y, color='blue', lw=10, alpha=0.5, label = 'Cm fault')
    # Alarm
    df['Sensor C alarm'][df['Sensor C alarm'] == 0] = np.nan
    ax[0].plot(-df['Sensor C alarm']*alarm_y, color='darkblue', lw=10, label = 'Cm alarm')
    ...
    y_range = ax[1].get_ylim()[1] - ax[1].get_ylim()[0]
    fault_y = lower_ylimit + 0.1 * y_range
    alarm_y = lower_ylimit + 0.2 * y_range
    # Fault
    df['Valve FW fault'][df['Valve FW fault'] == 0] = np.nan
    ax[1].plot(-df['Valve FW fault']*fault_y, color='orangered', lw=10, alpha=0.5, label = 'xFW fault')
    # Alarm
    df['Valve FW fault'][df['Valve FW fault'] == 0] = np.nan
    ax[1].plot(-df['Valve FW fault']*alarm_y, color='red', lw=10, label = 'xFW alarm')
...
# Economic data
def plotEconomicData(t, econ):
    # Plat cumulative profit
    cumulative_profit = np.nancumsum(econ['KPI']['values'])
    fig, ax = plt.subplots(1, 1, figsize=(10, 5))
    ax.plot(t['tvector'][:-1]/(3600*24), cumulative_profit, label=f'Cumulative profit\n(final value: {cumulative_profit[-1]:.0f})')
...
def find_start_end_points(df, condition, fill_gaps=False):
    ...
    starts = changes[changes > 0].index
    ends = changes[changes < 0].index
